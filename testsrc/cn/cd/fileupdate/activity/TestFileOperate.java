...
public class TestFileOperate extends AndroidTestCase
{
	...
	public void testSavePrivate() throws IOException
	{
		...
		FileOperate fileOperate = new FileOperate(this.getContext());
		// 测试写入数据
		fileOperate.saveContent("private.txt" ,"Private模式文件" ,Context.MODE_PRIVATE);
		// 再次写入内容
	...
	public void testSaveAppend() throws IOException
	{
		...
		FileOperate fileOperate = new FileOperate(this.getContext());
		// 测试写入数据
		fileOperate.saveContent("append.txt" ,"Append模式文件" ,Context.MODE_APPEND);
		// 再次写入数据
		fileOperate.saveContent("append.txt" ,"S-H-U" ,Context.MODE_APPEND);
	}

	/**
	 * 创建测试WORLD_WRITEABLE模式 本应用可以读写 允许其它应用进行文件的写入(不能读) 新写入的内容会覆盖原来
	 * 
	 * @throws IOException
	 */
	public void testSaveWriteable() throws IOException
	{
		...
		FileOperate fileOperate = new FileOperate(this.getContext());
		// 测试写入数据
		fileOperate.saveContent("writeable.txt" ,"WORLD_WRITEABLE模式文件" ,Context.MODE_WORLD_WRITEABLE);
		// 再次写入数据
		fileOperate.saveContent("writeable.txt" ,"S-H-U" ,Context.MODE_WORLD_WRITEABLE);
	}

	/**
	 * 创建测试WORLD_READABLE模式 本应用可以读写 允许其它应用进行文件的读取(不能写) 新写入的内容会覆盖原来的内容
	 * 
	 * @throws IOException
	 */
	public void testSaveReadable() throws IOException
	{
		...
		FileOperate fileOperate = new FileOperate(this.getContext());
		// 测试写入数据
		fileOperate.saveContent("readable.txt" ,"WORLD_READABLE模式文件" ,Context.MODE_WORLD_READABLE);
		// 再次写入数据
		fileOperate.saveContent("readable.txt" ,"S-H-U" ,Context.MODE_WORLD_READABLE);
	}

	/**
	 * 创建测试WORLD_READABLE和WORLD_WRITEABLE模式 权限叠加的方式 本应用可以读写 允许其它应用进行文件的读取和写入
	 * 新写入的内容会覆盖原来
	 * 
	 * @throws IOException
	 */
	public void testSaveRW() throws IOException
	{
		...
		FileOperate fileOperate = new FileOperate(this.getContext());
		// 测试写入数据
		fileOperate.saveContent("rw.txt" ,"WORLD_WRITEABLE+WORLD_READABLE模式文件" ,Context.MODE_WORLD_READABLE + Context.MODE_WORLD_WRITEABLE);
		// 再次写入数据
		fileOperate.saveContent("rw.txt" ,"S-H-U" ,Context.MODE_WORLD_READABLE + Context.MODE_WORLD_WRITEABLE);
	}
